...
postgresql_user_pwd = os.getenv("postgresql_user_pwd")
postgresql_db = 'spider_db'
postgresql_db_schema = 'public'

# sqlserver conf
sqlserver_ip = os.getenv("sqlserver_ip")
sqlserver_port = os.getenv("sqlserver_port")
sqlserver_user_name = os.getenv("sqlserver_user_name")
sqlserver_user_pwd = os.getenv("sqlserver_user_pwd")
sqlserver_db = os.getenv("sqlserver_db")
sqlserver_db_schema = 'dbo'
sqlserver_db_jdbc_url = os.getenv("sqlserver_db_jdbc_url")
...
def generate_user_comment_data():
    ...
    mssql_connection = (
        f"mssql+pymssql://{sqlserver_user_name}:{sqlserver_user_pwd}"
        f"@{sqlserver_ip}:{sqlserver_port}/{sqlserver_db}"
    )
    ...
    for i in records_list:
        try:
            ...
            logger.error(
                f"生成评论时出现异常: order_id={i.get('order_id')} user_id={i.get('user_id')} 原因: {e}",
                exc_info=True
            )
            continue
...
def push_feishu_msg(msg) -> None:
    ...
    feishu_url = f'https://www.feishu.cn/flow/api/trigger-webhook/{os.getenv("feishu_url")}'
    try:
        # 发送POST请求
...
def run_user_comment_generator():
    """运行用户评论生成器，每10秒调用一次"""
    while True:
        try:
            generate_user_comment_data()
            logger.info("用户评论生成完成，等待10秒后再次执行...")
            tm.sleep(10)  # 等待10秒
        except Exception as e:
            logger.error(f"用户评论生成器执行出错: {e}")
            tm.sleep(10)


def main(ds_ms):
    # collect_minio_user_device_2postgresql()
    # 启动用户评论生成器线程（每10秒执行一次）
    comment_thread = threading.Thread(target=run_user_comment_generator, daemon=True)
    comment_thread.start()
    logger.info("用户评论生成器线程已启动，每10秒执行一次")
    func_list = [
